 ...
 */
export default async function Breadcrumb({ items }: { items: Crumb[] }) {
  ...
  return (
    <nav aria-label={t("breadcrumb")}>
      <ol className="flex flex-wrap items-center gap-x-2 gap-y-1 font-mono text-xs text-muted">
        <li>
          <Link href="/" className="inline-flex min-h-[44px] min-w-[44px] items-center justify-center hover:text-accent">
            {t("home")}
          </Link>
        </li>
        {items.map((item, i) => {
          ...
          return (
            <li key={item.label} className="flex items-center gap-2">
              ...
              {item.href && !isLast ? (
                <Link
                  href={item.href}
                  className="inline-flex min-h-[44px] min-w-[44px] items-center justify-center hover:text-accent"
                >
                  {item.label}
                </Link>
              ) : (
